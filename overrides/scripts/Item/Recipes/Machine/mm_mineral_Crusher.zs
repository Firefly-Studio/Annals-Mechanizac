...
for i in 0 .. 22{
    if(i==22){
        var r1 = MachineUtil.buildMachine("cru"~i , "j_mineral_crusher", 360) as RecipePrimer;
        MachineUtil.inputResister(r1, false, true, false, true, 25, inp[i],1, defaultAura, defaultLiquid,100) as RecipePrimer;
        MachineUtil.outputResister(r1, false, true, false, false,false, 0, out[i],1,0,defaultAura, defaultLiquid,0) as RecipePrimer;
        r1.build();
    }else{
        var r1 = MachineUtil.buildMachine("cru"~i , "j_mineral_crusher", 320) as RecipePrimer;
        MachineUtil.inputResister(r1, false, true, false, true, 25, inp[i],1, defaultAura, defaultLiquid,100) as RecipePrimer;
